 ...
 * Class.forName("biznuvo.jdbcspy.DBProxyDriver");
 *
...
public class AbstractProxyDriver implements Driver {
    ...
    private static final Logger mTrace = LogManager.getLogger("jdbcspy.proxydriver");
    ...
    protected AbstractProxyDriver(final String driverClass) {
        mTrace.info("register {} as wrapper for driver {}", this.getClass().getName(), driverClass);

        try {
            final Class<?> c = Class.forName(driverClass);
            ...
            mTrace.error("Unable to register driver {}", driverClass, e);
        }
    }

    protected AbstractProxyDriver(final ClientProperties.Field dbType) {
        this(dbType.getStringValue());
    }
    ...
    public Connection connect(final String url, final Properties info) throws SQLException {
        final Connection c = uDriver.connect(url.substring(6), info);
